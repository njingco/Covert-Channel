...
int main(int argc, char **argv)
{
    ...
    /* Do the dirty work */
    forgepacket(source_host, dest_host, source_port, dest_port, filename, svr, ipid, seed);
    exit(0);
}
 ...
 * -----------------------------------------------------------------------*/
void forgepacket(unsigned int source_addr, unsigned int dest_addr, unsigned short source_port, unsigned short dest_port, char *filename, int svr, int ipid, int seed)
{
    if (svr == 0)
    {
        client(source_addr, dest_addr, source_port, dest_port, filename, ipid, seed);
        fprintf(stdout, "\nPackets Sent\n\n");
    }
    else
    {
        server(source_addr, source_port, dest_port, filename, ipid, seed);
        fprintf(stdout, "\nData Received\n\n");
    }
}
